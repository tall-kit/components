  ...
  Alpine.bind(el, {
    ...
    "x-data"() {
      return {
        ...
        __update() {
          this.$nextTick(() => {
            if (!el || !this.$refs.__content)
              return;
            computePosition2(el, this.$refs.__content, {
              placement: options.placement,
              middleware: [
                offset(options.offset),
                options.flip && flip(),
                options.shift && shift(),
                this.$refs.__arrow && arrow({ element: this.$refs.__arrow })
              ]
            }).then(({ x, y, placement, middlewareData }) => {
              this.__placement = placement;
              Object.assign(this.$refs.__content.style, {
                left: `${x}px`,
                top: `${y}px`
              });
              if (this.$refs.__arrow) {
                ...
                Object.assign(this.$refs.__arrow.style, {
        ...
        __show(withDelay) {
          const handle = () => {
            this.__isShown = true;
            this.__update();
            this.__autoupdateCleanup = autoUpdate(this.$el, this.$refs.__content, () => {
              this.__update();
            });
          };
          withDelay ? this.__delayInterval = setTimeout(handle, options.delay) : handle();
        },
        __hide() {
